...
| Стили | CSS Modules, сначала под телефон, без UI-библиотеки |
...
└── pages/
    ├── LandingPage.tsx
    ├── CabinetLayout.tsx     # шапка (имя, «Выйти») + нижняя навигация на телефоне / вкладки на десктопе
    └── NotFoundPage.tsx
```
...
Общее: сначала под телефон, от 360 px. Навигация — нижняя панель на телефоне, вкладки сверху на ширине ≥ 768 px. Результат изменений показывается тостом, ошибки загрузки — блоком `ErrorState` с кнопкой «Повторить».

### 8.1 Комплименты (`/app/compliments`)

- Запрос `['compliments']` → `GET /api/compliment`. Сортировка на клиенте: новые сверху.
- Карточка: заголовок, текст (обрезается до 3 строк, раскрывается по нажатию), статус «В очереди» или «Отправлен» (`isBeenPushed`), дата создания.
- Действия карточки: «Изменить» (модалка с формой), «Удалить» (подтверждение), «Вернуть в очередь» (только у отправленных: `PUT` с `isBeenPushed=false`).
- «Добавить» открывает форму: заголовок (обязательно, ≤ 100 символов) и текст (обязательно, ≤ 500). Лимиты те же, что в базе. У полей счётчики символов и подсказка «на экране блокировки видно 2–4 строки».
- «Добавить списком» (`BulkImport`): поле «Заголовок для всех» (по умолчанию «Комплимент для тебя 💌», ≤ 100) и поле текста. Каждая непустая строка — отдельный комплимент, пробелы по краям обрезаются. Строки длиннее 500 символов и пустой результат дают ошибку до отправки. Предпросмотр: «Будет добавлено N». Отправка → `POST /api/compliment/batch`.
...
- Строка: устройство (`parseUserAgent`: «iPhone · Safari», «Android · Chrome», «Windows · Chrome», «Mac · Safari», иначе «Неизвестное устройство»), дата подписки, бейдж «Активна» или «Отключена».
